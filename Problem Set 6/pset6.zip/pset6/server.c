...
char* indexes(const char* path)
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (str_equal(namelist[i]->d_name, "index.html"))
        {
            has_index_html = true;
        }

        if (str_equal(namelist[i]->d_name, "index.php"))
        {
            has_index_php = true;
        }
    ...
    if (has_index_php)
    {
        index_size = strlen(path) + strlen("/index.php") + 1;
    ...
    if (has_index_html)
    {
        index_size = strlen(path) + strlen("/index.html") + 1;
        char*  index = malloc(index_size);
        strcpy(index, path);
...
const char* lookup(const char* path)
{
    // if path does not exist don't bother
    if(access(path, F_OK) == -1)
    {
        return NULL;
    }
    ...
    char* path_end = period + 1;
    mime types[] = {
        { .ends_with = "css", .type =  "text/css"},
        { .ends_with = "html", .type =  "text/html"},
        { .ends_with = "gif", .type =  "image/gif"},
        { .ends_with = "ico", .type =  "image/x-icon"},
        { .ends_with = "jpg", .type =  "image/jpeg"},
        { .ends_with = "js", .type =  "text/javascript"},
        { .ends_with = "php", .type =  "text/x-php"},
        { .ends_with = "png", .type =  "image/png"},
        // must be last
        { .ends_with = NULL, .type = NULL}
    };

    for (int i = 0; types[i].type != NULL; i++)
    {
        if (str_equal(path_end, types[i].ends_with))
        {
            return types[i].type;
        }
    }
    // error out for non supported extension
    return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    int token_size = delimeter - line;
    char method[token_size];
    strncpy(method, line, token_size);
    method[token_size] = '\0';
    ...
    token_size = delimeter - HEAD;
    char request_target[token_size];
    strncpy(request_target, HEAD, token_size);
    request_target[token_size] = '\0';

    // disallow if request-target does not begin with '/'
    if (request_target[0] != '/')
    {
        error(501);
        return false;
    ...
    // disallow if request-target contains '"'
    ...
    token_size = delimeter - HEAD;
    char http_version[token_size];
    strncpy(http_version, HEAD, token_size);
    http_version[token_size] = '\0';

    // only allow HTTP/1.1 version
    if (!str_equal(http_version, "HTTP/1.1"))
    {
        error(505);
        return false;
    }

    // rest of string must end with ctrl-f
    HEAD = delimeter + 1;
    if (!str_equal(HEAD, "\n"))
    {
        error(400);
    ...
    // parse abs_path and query
    HEAD = request_target;
    char* query_token = strchr(request_target, '?');
    if (query_token == NULL)
    {
        strcpy(abs_path, HEAD);
        strcpy(query, "");
    }
    else
    {
        // parse abs_path
        token_size = query_token - HEAD;
        char path[token_size];
        strncpy(path, request_target, token_size);
        path[token_size] = '\0';
